...
class DimerEigenmodeSearch:
    ...
    * Kastner and Sherwood, JCP 128, 014106 (2008).
    ...
    def __init__(self, atoms, control=None, eigenmode=None, basis=None,
                 **kwargs):
        if hasattr(s, 'get_eigenmode'):
            self.s = s
        else:
            e = 'The s object must be a MinModes object'
            raise TypeError(e)
        self.basis = basis

        if eigenmode is None:
            self.eigenmode = self.s.get_eigenmode()
        else:
    ...
    def get_rotational_force(self):
        ...
        if self.basis is not None:
            if len(self.basis) == len(self.s) and len(self.basis[0]) == \
    ...
    def update_curvature(self, curv = None):
        """Update the curvature in the MinModes object."""
        if curv:
            self.curvature = curv
        else:
    ...
    def update_eigenmode(self, eigenmode):
        """Update the eigenmode in the MinModes object."""
        self.eigenmode = eigenmode
    ...
    def update_center_forces(self):
        ...
        self.s.set_positions(self.pos0)
        self.forces0 = self.s.get_forces(real = True)
        self.energy0 = self.s.get_potential_energy()
    ...
    def update_virtual_forces(self, extrapolated_forces = False):
        ...
        if extrapolated_forces:
            ...
            self.forces1 = self.s.get_forces(real = True, pos = self.pos1)
        ...
        if self.control.get_parameter('use_central_forces'):
            ...
            self.forces2 = self.s.get_forces(real = True, pos = self.pos2)
    ...
    def set_up_for_eigenmode_search(self):
        ...
        self.pos0 = self.s.get_positions()
